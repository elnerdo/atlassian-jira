...
@task
def restore(destination='jirabackup', revision='latest'):
...
def do_snapshot():
    ...
    if backup_vol:
        conn.create_snapshot(backup_vol.id, description='jira-data-snap')
...
def do_bup_restore(destination, revision):
    restored = local('BUP_DIR=$(pwd)/backup bup restore -C {0} '
                     'jirabackup/{1}/$(pwd)'.format(destination, revision))
...
def do_psql_dump():
    ...
    if not os.path.exists(os.path.join(os.getcwd(), 'dumps')):
        local('mkdir dumps')
    local('mv jira.dump dumps/jira.dump')
