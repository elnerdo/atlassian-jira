...
def do_bup_backup():
    ...
    backuped = local('BUP_DIR=$(pwd)/backup bup save -n jirabackup $(pwd)')
...
def do_bup_restore():
    restored = local('BUP_DIR=$(pwd)/backup bup restore -C jirabackup '
                     'jirabackup/latest/$(pwd)')
...
def do_psql_dump():
    print 'dumping...'
    dumped = local('docker run -t --rm --link jira_database_1:db '
                   '-v $(pwd):/tmp postgres sh -c \'pg_dump -U jira -h '
                   '"$DB_PORT_5432_TCP_ADDR" -w jira > /tmp/jira.dump\'')
...
def stop_docker_container():
    print 'stoping...'
    stopped = local('sudo docker-compose stop jira')
    if stopped.return_code != 0:
        print 'An error occurred while stoping container jira. Aborting...'
        sys.exit(1)
    print 'stopped...'
...
def start_docker_container():
    print 'starting...'
    started = local('sudo docker-compose start jira')
    if started.return_code != 0:
        print 'An error occurred while starting container jira. Aborting...'
        sys.exit(1)
    print 'started...'
